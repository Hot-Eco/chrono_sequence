...
m1 <- gam(log(percentC) ~ s(tsf.years, by = dft, bs = "cr") + fireType + depth + microsite,
          data = DAT,
          method = "REML")
...
ggplot(data = dat, aes(x = fitted, y = resid)) +
  ...
  labs(x = Labels$fit, y = Labels$res.p) +
...
ggplot(data = dat, aes(x = fireType, y = resid)) +
  ...
  labs(x = Labels$firetype, y = Labels$res.p) +
...
ggplot(data = dat, aes(x = microsite, y = resid)) +
  ...
  labs(x = Labels$microsite, y = Labels$res.p) +
...
ggplot(data = dat2, aes(x = order, y = resid)) +
  ...
  labs(x = Labels$site.by.res, y = Labels$res.p) +
...
m2 <- gam(log(percentC) ~ s(tsf.years, by = dft, bs = "cr") + fireType + depth + microsite + 
...
dat$resid <- resid(m2, type = "pearson")
dat$fitted <- fitted(m2)
...
The second model, incorporating site random effect, is clearly preferred.
...
fit <- do_predict(m2, dat.predict, "percentC", exp)
...
m3 <- gam(log(percentRPC) ~ s(tsf.years, by = dft) + 
...
dat$resid <- resid(m3, type = "pearson")
dat$fitted <- fitted(m3)
...
The offender is sample number 166. Its value of RPC is very low compared to the other two replicates. Checked with Robert and he suggests that this was most likely a problem with the digestion. The sample will have had little influence on the model, but to be virtuous we discard it and re-run...
...
**Check with Robert whether this is sensible.**
...
fit <- do_predict(m3, dat.predict, "percentRPC", exp)
...
m4 <- gam(log(propRPC) ~ s(tsf.years, by = dft) + 
            fireType + depth + microsite +
            s(fSiteId, bs = "re", by = flag),
          data = dat,
          method = "REML")
...
dat$resid <- resid(m4, type = "pearson")
dat$fitted <- fitted(m4)
...
fit <- do_predict(m4, dat.predict, "propRPC", exp)
