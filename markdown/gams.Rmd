...
# Output folder for publication quality graphs
PUBLICATION_GRAPHS_PATH <- mdPath("outputs")

# Set environment variable to local Ghostscript installation to
# allow us to embed fonts in the EPS files for publication graphs
...
A grey-scale version of the same graph for publication.

(To use transparency (ie. alpha) we have to save as PDF then convert to EPS outside of R)

```{r}

ggplot(data = fit, aes(x = tsf.years, y = percentC)) +
  geom_ribbon(aes(ymin = lwr, ymax = upr, group = fireType),
              fill = "grey", alpha = 0.5) +
  
  geom_point(data = DAT, aes(shape = fireType), size = POINT_SIZE) +
  
  scale_shape_manual(name = Labels$firetype, values = c(16, 1)) +
  
  geom_line(aes(linetype = fireType), size = LINE_SIZE) +
  
  scale_linetype_discrete(name = Labels$firetype) +
...
fname <- file.path(PUBLICATION_GRAPHS_PATH, "sawyer_figure_2.pdf")
...
Grey-scale version of graph for publication.
...
fname <- file.path(PUBLICATION_GRAPHS_PATH, "sawyer_figure_3.pdf")
...
Grey-scale version of same graph for publication:
...
fname <- file.path(PUBLICATION_GRAPHS_PATH, "sawyer_figure_4.pdf")
...
fname <- file.path(PUBLICATION_GRAPHS_PATH, "sawyer_figure_5.pdf")
